...
Before(async function(){
  ...
  await driver.manage().setTimeouts({implicit:5000,pageLoad:10000,script:3000});
});
...
Given("I am navigated to {string}", async function (url) {
  await driver.get(url);
});

When("I select Country as {string}", async function (country) {
  await driver.findElement(By.css("a#country-btn")).click();
  await driver.findElement(By.xpath(`//a[contains(@class,'country')]/span[contains(text(),'${country}')]/parent::a`)).click();
});

Then("I verify the currency is {string}", async function (currency) {
  var trialCostText = await driver.findElement(By.css("div.trial-cost")).getText();
...
Then("I verify the prices of each package", async function (dataTable) {
  ...
  for(var i=0;i<types.length;i++){
    var actualPriceText = await driver.findElement(By.xpath(`//div[@class='price' and contains(@id,'${types[i]}')]`)).getText();
